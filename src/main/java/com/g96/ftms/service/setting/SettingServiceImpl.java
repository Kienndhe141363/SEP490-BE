...
public class SettingServiceImpl implements SettingService {
    ...
    public ApiResponse<Settings> createSetting(SettingRequest.SettingAddRequest model) {
        ...
        if (model.getSettingGroup() == SettingGroupEnum.GENERATION) {
            Optional<Generation> existingGeneration = generationRepository.findByGenerationName(model.getSettingName());
            if (existingGeneration.isPresent()) {
                throw new AppException(HttpStatus.BAD_REQUEST, ErrorCode.DUPLICATE_SETTING);
            }

            Generation generation = existingGeneration.orElseThrow(()
                    -> new AppException(HttpStatus.BAD_REQUEST, ErrorCode.GENERATION_NOT_FOUND));
        ...
        if (model.getSettingGroup() == SettingGroupEnum.ROOM) {
            // Check if the room setting name already exists
            Optional<Room> existingRoom = roomRepository.findByRoomName(model.getSettingName());
            if (existingRoom.isPresent()) {
                throw new AppException(HttpStatus.BAD_REQUEST, ErrorCode.DUPLICATE_SETTING);
            }

            Room room = existingRoom.orElseThrow(()
